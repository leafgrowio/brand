...
This kit is a downstream build of the Leaf design system (`system/` in the public GitHub repo `leafgrowio/brand`, v2.1). The canonical spec is `system/DESIGN.md` in that repo; inside the Leaf plugin the same document is mirrored as the **design** slice of the `leaf-context` skill (at `brand/files/DESIGN.md`).
