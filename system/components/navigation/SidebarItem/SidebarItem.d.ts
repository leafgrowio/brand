 ...
 * The only Coral signal in the app shell: active items get a tint fill and
 * Ink text and icon, at the same padding as the resting state. Collapses
 * to a 36×36 icon-only tile when the sidebar is minimised.
 *
 * @startingPoint
 */
export interface SidebarItemProps {
  ...
  label: string;
  /** Marks this item as the current page: tint fill, Ink text/icon, `aria-current="page"`. */
  active?: boolean;
